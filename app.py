...
try:
    ...
    class LinearRegression:  # simple 1D fallback
        ...
        def fit(self, X, y):
            # Expect X shape (n,1) or (n,), y shape (n,)
            ...
            if x.size == 0 or y.size == 0:
                ...
                self.intercept_ = float(_np.nan)
                return self
            x_mean = x.mean()
            y_mean = y.mean()
            denom = _np.mean((x - x_mean) ** 2)
            if denom == 0:
                b = 0.0
            else:
        ...
        def score(self, X, y):
            x = _np.asarray(X).reshape(-1)
            y = _np.asarray(y).reshape(-1)
            mask = (~_np.isnan(x)) & (~_np.isnan(y)) & (~_np.isnan(self.intercept_)) & (~_np.isnan(self.coef_[0]))
            x = x[mask]
            y = y[mask]
            if x.size == 0 or y.size == 0:
                return 0.0
            y_hat = self.intercept_ + self.coef_[0] * x
            ss_res = _np.sum((y - y_hat) ** 2)
...
if not up:
    st.info("No subiste archivo. Cargando **datos_ejemplo.csv** por defecto (puedes reemplazarlo subiendo tu propio archivo).")
    try:
        raw = pd.read_csv("datos_ejemplo.csv", header=0)
        up = type("obj", (), {"name":"datos_ejemplo.csv"})()  # dummy to mantener flujo
    except Exception as e:
        st.error(f"No se pudo cargar datos_ejemplo.csv automáticamente: {e}")
        st.stop()
